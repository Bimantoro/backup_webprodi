...
class Page extends CI_Controller {
	...
	function index()	{
		...
		$data['side'] = $this->page_model->daftar_side_menu_lab();
	...
	function mata_kuliah($kd_kur="",$kd_mk=""){
		if($kd_mk){			
			...
			$nim="";
			$this->breadcrumb->append_crumb('Beranda', base_url());
			$this->breadcrumb->append_crumb('Kurikulum', site_url('id/page/kurikulum'));
	...
	function detil_dosen($kd_dosen="",$history=""){
		...
		if($dosen){
			...
			$this->breadcrumb->append_crumb('Beranda', base_url());
			$this->breadcrumb->append_crumb('Dosen', site_url($lang.'/page/dosen'));
	...
	function pedoman_akademik($id=0){
		...
		if( !$data ){
			...
			if($data_page->num_rows()>0){
				$this->breadcrumb->append_crumb('Beranda', base_url());;
				$this->breadcrumb->append_crumb($page->nama_dokumen, '/');
