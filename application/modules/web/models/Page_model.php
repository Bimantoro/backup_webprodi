...
class Page_model extends CI_Model {
	...
	public function daftar_berita($limit=1,$offset=0){
		return $w = $this->db->query("select * from berita WHERE kode_unit='".$this->get_session_unit()."' AND kode_bahasa='".$this->lang."'order by id_berita DESC limit ".$offset.",".$limit."" )->result();
	} 
	...
	public function daftar_pengumuman($limit=1,$offset=0){
		return $w = $this->db->query("select * from pengumuman WHERE kode_unit='".$this->get_session_unit()."' AND kode_bahasa='".$this->lang."' order by id_pengumuman DESC limit ".$offset.",".$limit."" )->result();
		
	}  
		
	public function daftar_agenda($limit=1,$offset=0){
		return $w = $this->db->query("select * from agenda WHERE kode_unit='".$this->get_session_unit()."' AND kode_bahasa='".$this->lang."' order by id_agenda DESC limit ".$offset.",".$limit."" )->result();
	}
	...
	public function daftar_kolom($limit=1,$offset=0){
		return $w = $this->db->query("select * from kolom WHERE kode_unit='".$this->get_session_unit()."' AND kode_bahasa='".$this->lang."' order by id_kolom DESC limit ".$offset.",".$limit."" )->result();
	}
	public function daftar_liputan($limit=1,$offset=0){
		return $w = $this->db->query("select * from liputan WHERE kode_unit='".$this->get_session_unit()."' AND kode_bahasa='".$this->lang."' order by id_liputan DESC limit ".$offset.",".$limit."" )->result();
	}
	...
	public function generate_index_berita($limit,$offset,$filter=array()){
		...
		if($filter['tahun']!=null) $where.=" AND year(tgl_posting)='".$filter['tahun']."'";
		if($filter['bulan']!=null) $where.=" AND month(tgl_posting)='".$filter['bulan']."'";
		return $w = $this->db->query("select * from berita ".$where." order by id_berita DESC limit ".$offset.", ".$limit."")->result();
	}
	...
	function get_arsip_dokumen($limit,$offset,$filter=array()){
		...
		$config['base_url'] = site_url('web/dokumen/index/');
		$config['total_rows'] = $tot_hal->num_rows();
		$config['per_page'] = $limit;
		$config['uri_segment'] = 4;
	...
	function get_berita_terkait($filter = '', $offset = 0, $limit = 5){
		$query=$this->db->query("SELECT id_berita, judul, tgl_posting from berita where kode_unit='".$this->get_session_unit()."' AND ".$filter." ORDER BY tgl_posting desc LIMIT ".$offset.", ".$limit);
		return $query->result(); 
	}

	function get_berita_terpopuler($offset = 0, $limit = 5){
		$query=$this->db->query("SELECT id_berita, judul, tgl_posting from berita where kode_unit='".$this->get_session_unit()."' ORDER BY counter desc LIMIT ".$offset.", ".$limit);
		return $query->result();
	}

	function get_pengumuman_terkait($filter = '', $offset = 0, $limit = 5){
		$query=$this->db->query("SELECT id_pengumuman, nama_pengumuman, tgl_posting from pengumuman where kode_unit='".$this->get_session_unit()."' AND ".$filter." ORDER BY tgl_posting desc LIMIT ".$offset.", ".$limit);
		return $query->result();
	}

	function get_pengumuman_terpopuler($offset = 0, $limit = 5){
		$query=$this->db->query("SELECT id_pengumuman, nama_pengumuman, tgl_posting from pengumuman where kode_unit='".$this->get_session_unit()."' ORDER BY counter desc LIMIT ".$offset.", ".$limit);
		return $query->result();
	}
	
	function get_liputan_terkait($filter = '', $offset = 0, $limit = 5){
		$query=$this->db->query("SELECT id_liputan, judul, tgl_posting from liputan where kode_unit='".$this->get_session_unit()."' AND ".$filter." ORDER BY tgl_posting desc LIMIT ".$offset.", ".$limit);
		return $query->result();
	}

	function get_liputan_terpopuler($offset = 0, $limit = 5){
		$query=$this->db->query("SELECT id_liputan, judul, tgl_posting from liputan where kode_unit='".$this->get_session_unit()."' ORDER BY counter desc LIMIT ".$offset.", ".$limit);
		return $query->result();
	}

	function get_kolom_terkait($filter = '', $offset = 0, $limit = 5){
		$query=$this->db->query("SELECT id_kolom, judul, tgl_posting from kolom where kode_unit='".$this->get_session_unit()."' AND ".$filter." ORDER BY tgl_posting desc LIMIT ".$offset.", ".$limit);
		return $query->result();
	}

	function get_kolom_terpopuler($offset = 0, $limit = 5){
		$query=$this->db->query("SELECT id_kolom, judul, tgl_posting from kolom where kode_unit='".$this->get_session_unit()."' ORDER BY counter desc LIMIT ".$offset.", ".$limit);
		return $query->result();
	}

	function get_agenda_terkait($filter = '', $offset = 0, $limit = 5){
		$query=$this->db->query("SELECT id_agenda, nama_agenda, tgl_posting from agenda where kode_unit='".$this->get_session_unit()."' AND ".$filter." ORDER BY tgl_posting desc LIMIT ".$offset.", ".$limit);
		return $query->result();
	}

	function get_agenda_terpopuler($offset = 0, $limit = 5){
		$query=$this->db->query("SELECT id_agenda, nama_agenda, tgl_posting from agenda where kode_unit='".$this->get_session_unit()."' ORDER BY counter desc LIMIT ".$offset.", ".$limit);
		return $query->result();
	}
